...
if __name__ == '__main__':
    ...
    images = sorted(os.listdir(osp.join(train_path, "img")))
    labels = sorted(os.listdir(osp.join(train_path, "json")))
    ...
    for idx, (img_name, label_name) in enumerate(tqdm.tqdm(zip(images, labels), total=len(images))):
        ...
        overlay_mask = np.ones_like(rotated_image)
        
        ## Simple paste
        test_image[center_y - crop_size // 2: center_y + crop_size // 2, center_x - crop_size // 2 : center_x + crop_size // 2] += overlay_mask 
    ...
    Image.fromarray(test_image.astype(np.uint8)).save(osp.join(save_dir, 'overlay_heatmap.png'))
